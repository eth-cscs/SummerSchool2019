...
template <typename T>
__global__
void axpy(int n, double alpha, const T* x, T* y) {
    int i = threadIdx.x;
    y[i] += alpha*x[i];
}

// call with 1024 threads always, i.e. <<<1, 1024>>>
template <typename T>
__global__
void axpy_bign(int n, double alpha, const T* x, T* y) {
    int i = threadIdx.x;
    while (i<n) {
        y[i] += alpha*x[i];
        i += 1024;
...
template <typename T>
__global__
void axpy_final(int n, double alpha, const T* x, T* y) {
...
int main(int argc, char** argv) {
    ...
    unsigned nblocks = (n + block_dim -1)/block_dim;
    ...
    // with one thread block
    //axpy<<<1, n>>>(n, 2.0, x_device, y_device);
    //axpy_bign<<<1, 1024>>>(N, 2.0, X_DEvice, y_device);

    // with multiple thread blocks
    axpy_final<<<nblocks, block_dim>>>(n, 2.0, x_device, y_device);
